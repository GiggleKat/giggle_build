...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^gigglekat_") ; then
       GIGGLEKAT_BUILD=$(echo -n $1 | sed -e 's/^gigglekat_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $GIGGLEKAT_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       GIGGLEKAT_BUILD=
    fi
    export GIGGLEKAT_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${GIGGLEKAT_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    GIGGLEKAT_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/gigglekat/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch gigglekat_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var GIGGLEKAT_VERSION)
        ZIPFILE=gigglekat-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.gigglekat.device=$GIGGLEKAT_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $GIGGLEKAT_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $GIGGLEKAT_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/gigglekat/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
